( function ( global ) {
  ...
  function clearScript () {
    // document.querySelectorAll return a NodeList, which is not a Array, so use [].slice turn it into a array
  ...
  function loadExample ( evt ) {
    ...
    clearScript()
    file.examples[ exampleName ].forEach( function ( blcok ) {
      scriptElem.appendChild( Block.create.apply( null, block ) )
    } )
    Menu.runSoon()
  }
  ...
  document.querySelector( '.laod-action' ).addEventListener( 'click', loadFile, false )
  document.querySelector( '.choose-example' ).addEventListener( 'click', loadExample, false )
